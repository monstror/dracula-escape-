...
void setup() {
  ...
  while (digitalRead(IN_PUT) == LOW){yield();}
...
void loop() {
  ...
  if (led_pos_1 == 3 && led_pos_2 == 1 && led_pos_3 == 2 && k_final == 0){ // если все энкодеры стоят в нужном положении
      final_pos = millis(); //засекаем время
      k_final = 1;     //ставим флаг
  }
  if (led_pos_1 != 3 || led_pos_2 != 1 || led_pos_3 != 2) k_final = 0; //если нет, то флаг снимаем
  if (k_final == 1 && millis() - final_pos > 1500){ //если усё норм, то
      ...
      for (i = 0; i < 120; i++){
           ...
           strip.setPixelColor(5, strip.Color(r, g, 0));
           strip.setPixelColor(10, strip.Color(r, g, 0));
           strip.show();
